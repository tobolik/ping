...
// Hybridní úložiště - databáze jako primární, localStorage jako fallback
const storage = {
    // Uložení do databáze i localStorage
    async save(data) {
        try {
            // Uložení do localStorage jako fallback
            localStorage.setItem(STORAGE_KEY, JSON.stringify(data));
            
            // Uložení do databáze (pokud je dostupná)
            await this.saveToDatabase(data);
        } catch (error) {
            console.warn('Chyba při ukládání do databáze, data uložena pouze lokálně:', error);
            // Data jsou již uložena v localStorage
        }
    },
    
    // Načtení z databáze nebo localStorage
    async load() {
        try {
            ...
            console.warn('Chyba při načítání z databáze, načítám z localStorage:', error);
        }
        ...
        return { playerDatabase: [], tournaments: [], settings: { soundsEnabled: true } };
    },
    
    // Uložení do databáze
    async saveToDatabase(data) {
        // Uložení hráčů
        for (const player of data.playerDatabase) {
            if (player.id && player.id > 1000000) { // localStorage ID (timestamp)
                await api.players.create(player);
            } else {
                await api.players.update(player);
            }
        }
        
        // Uložení turnajů
        for (const tournament of data.tournaments) {
            if (tournament.id && tournament.id > 1000000) { // localStorage ID
                await api.tournaments.create(tournament);
            } else {
                await api.tournaments.update(tournament);
            }
        }
        
        // Uložení nastavení
        await api.settings.update('sounds_enabled', data.settings.soundsEnabled.toString());
    },
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadData() {
        try {
            ...
            state.playerDatabase = data.playerDatabase || [];
            state.tournaments = data.tournaments || [];
            state.settings.soundsEnabled = data.settings?.soundsEnabled || true;
            document.getElementById('sound-toggle').checked = state.settings.soundsEnabled;
            console.log('Data načtena:', { players: state.playerDatabase.length, tournaments: state.tournaments.length });
        } catch (error) {
    ...
    async function saveState() {
        try {
            await storage.save({
                playerDatabase: state.playerDatabase,
            ...
            showError('Nepodařilo se uložit data: ' + error.message);
        }
    }
    ...
    function generateMatchesForTournament(playerIds) {
        ...
        for (let i = 0; i < playerIds.length; i++) {
            for (let j = i + 1; j < playerIds.length; j++) {
                matches.push({
                    id: `${playerIds[i]}-${playerIds[j]}-${Date.now()}`,
    ...
    const renderTournamentScreen = async () => {
        ...
        const renderMatch = (m, isCompleted) => {
            ...
            const p1Color = playerColors[t.playerIds.indexOf(m.player1Id) % playerColors.length];
            const p2Color = playerColors[t.playerIds.indexOf(m.player2Id) % playerColors.length];
    ...
    const recalculateServiceState = (match, pointsToWin) => {
        ...
        if (pointsToWin === 11 && match.score1 >= 10 && match.score2 >= 10) {
            ...
            if (Math.floor(pointsInDeuce / 1) % 2 === 0) {
                match.servingPlayer = (match.firstServer === 1) ? 1 : 2;
            } else {
                match.servingPlayer = (match.firstServer === 1) ? 2 : 1;
            }
            return;
        }

        if (totalScore === 0) {
            match.servingPlayer = match.firstServer;
            return;
        }
    ...
    const allActions = {
        ...
        'save-player': async (target) => {
            ...
            if (playerId) {
                const playerIndex = state.playerDatabase.findIndex(p => p.id === playerId);
                state.playerDatabase[playerIndex] = { ...state.playerDatabase[playerIndex], ...playerData };
            } else {
                state.playerDatabase.push({ id: Date.now(), ...playerData });
            }
        ...
        'delete-player': async (target) => {
            ...
            if (isPlayerInTournament) {
                alert('Hráče nelze smazat, protože je součástí jednoho nebo více turnajů.');
                return;
            }

            if (confirm('Opravdu chcete smazat tohoto hráče z databáze?')) {
                state.playerDatabase = state.playerDatabase.filter(p => p.id !== playerId);
                await saveState();
                renderPlayerDbScreen();
            }
        },
        ...
        'create-tournament': async () => {
            ...
            try {
                ...
                console.log('Turnaj vytvořen:', newTournament);
                closeModal();
                renderMainScreen();
            } catch (error) {
        ...
        'toggle-lock-main': async (target) => {
            const t = getTournament(parseInt(target.dataset.id));
        ...
        'play-match': async (target) => {
            ...
            if (!m.firstServer) {
                const p1 = getGlobalPlayer(m.player1Id);
                const p2 = getGlobalPlayer(m.player2Id);
                renderGameScreen(`<div class="bg-white p-6 rounded-xl shadow-sm space-y-6 text-center"><h1 class="text-2xl font-bold">${p1.name} vs ${p2.name}</h1><p class="text-gray-500">Hraje se na ${t.pointsToWin} bodů</p><div><h2 class="text-lg font-semibold mb-3">Kdo má první podání?</h2><div class="grid grid-cols-2 gap-4"><button data-action="set-first-server" data-player="1" class="p-4 rounded-lg font-bold text-white ${playerColors[t.playerIds.indexOf(m.player1Id) % playerColors.length]}">${p1.name}</button><button data-action="set-first-server" data-player="2" class="p-4 rounded-lg font-bold text-white ${playerColors[t.playerIds.indexOf(m.player2Id) % playerColors.length]}">${p2.name}</button></div></div><button data-action="back-to-tournament" class="btn btn-secondary w-full mt-4">Zpět</button></div>`);
            } else {
                renderGameBoard();
            }
        },
        'set-first-server': async (target) => {
            const m = state.currentMatch;
            m.firstServer = parseInt(target.dataset.player);
            recalculateServiceState(m, getTournament().pointsToWin);
    ...
    function renderGameBoard() {
        ...
        renderGameScreen(`<div class="h-full flex flex-col"><header class="bg-white/90 backdrop-blur-sm p-3 shadow-sm text-center flex justify-between items-center w-full z-10 flex-shrink-0"><button data-action="back-to-tournament" class="btn btn-secondary !p-2 w-24">Přerušit</button><div class="flex-grow"><p class="text-sm text-gray-500">Hraje se na ${t.pointsToWin} bodů</p><p class="font-semibold">Podání: ${m.servingPlayer ? (m.servingPlayer === 1 ? p1.name : p2.name) : '...'}</p></div><div class="w-24 flex justify-end"><button data-action="toggle-sound-ingame" class="btn btn-secondary !p-0 h-10 w-10 text-lg">${state.settings.soundsEnabled ? '<i class="fa-solid fa-volume-high"></i>' : '<i class="fa-solid fa-volume-xmark"></i>'}</button></div></header><div class="flex-grow flex flex-col md:flex-row"><div ${canAddPoints ? `data-action="add-point" data-player="1"` : ''} class="player-score-box ${p1Color} p-4 text-white relative flex flex-col items-center justify-center flex-grow ${canAddPoints ? 'active-pointer cursor-pointer' : ''}">${m.servingPlayer === 1 ? `<div class="absolute top-4 left-4 w-10 h-10 rounded-full bg-yellow-400/90 text-2xl flex items-center justify-center">🏓</div>` : ''}<button data-action="subtract-point" data-player="1" class="absolute top-4 right-4 w-12 h-12 bg-black/20 rounded-full text-2xl">-1</button><div class="text-2xl md:text-3xl font-bold">${p1.name}</div><div class="text-8xl md:text-9xl font-extrabold my-4">${m.score1}</div><div class="text-sm opacity-80">${canAddPoints ? 'Klepněte pro bod' : '&nbsp;'}</div></div><div ${canAddPoints ? `data-action="add-point" data-player="2"` : ''} class="player-score-box ${p2Color} p-4 text-white relative flex flex-col items-center justify-center flex-grow ${canAddPoints ? 'active-pointer cursor-pointer' : ''}">${m.servingPlayer === 2 ? `<div class="absolute top-4 left-4 w-10 h-10 rounded-full bg-yellow-400/90 text-2xl flex items-center justify-center">🏓</div>` : ''}<button data-action="subtract-point" data-player="2" class="absolute top-4 right-4 w-12 h-12 bg-black/20 rounded-full text-2xl">-1</button><div class="text-2xl md:text-3xl font-bold">${p2.name}</div><div class="text-8xl md:text-9xl font-extrabold my-4">${m.score2}</div><div class="text-sm opacity-80">${canAddPoints ? 'Klepněte pro bod' : '&nbsp;'}</div></div></div>${winner ? `<div class="absolute bottom-4 left-4 right-4 bg-white p-6 rounded-xl shadow-lg text-center space-y-3 z-20"><div class="text-3xl">🏆</div><h2 class="text-2xl font-bold">Vítěz: ${winner === 1 ? p1.name : p2.name}!</h2><p class="text-gray-500">Výsledek: ${m.score1} : ${m.score2}</p><button data-action="save-match-result" class="btn btn-primary w-full">Uložit výsledek</button></div>` : ''}</div>`);
    }
